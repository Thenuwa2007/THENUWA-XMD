...
cmd({
  ...
  try {
    ...
    await conn.sendMessage(from, {
      ...
      caption: `📥 *THENUWA XMD FB DOWNLOADER..🚀*\n\n*QUAILTY•${videoData.quality}*\n\n🔗 *THENUWA XMD*`
    }, { quoted: m });
...
async (conn, mek, m, { from, args, q, reply, react }) => {
    try {
        ...
        const captionText = `*THENUWA XMD IG DOWNLOADER🚀*` +
                            `📸 *Instagram Post* 📸\n\n` +
                            `👤 *User:* ${fullname} (@${username})\n` +
                            `❤️ *Likes:* ${likes}\n💬 *Comments:* ${comments}\n👥 *Followers:* ${followed}\n` +
                            `📝 *Caption:*\n${caption || "THE THENUWA XMD API."}`;
...
async (conn, mek, m, { from, args, q, reply }) => {
    try {
        ...
        reply("*_THENUWA XMD DOWNLOADING TIKTOK VIDEO , PLEASE WAIT...🚀_*");
        ...
        const caption = `🎵 *THENUWA XMD TIKTOK VIDEO* 🎵\n\n` +
